...
import UploadPGNModalCrafty from '@/app/components/CraftyPgnModal';
...
import { terminateCraftyEngine, type CraftyAnalysisResult } from '@/app/utils/crafty-client';
...
// Crafty's analysis result shape is identical to the Stockfish one, so the
// rest of the component (history table, eval bar, AI explainer) needs no
// structural changes — only the engine call underneath the modal differs.
type AnalysisResult = CraftyAnalysisResult;

const GameAnalyzerCrafty: React.FC = () => {
    // Game state
    ...
    // PGN analysis integration (Crafty)
    ...
    // Crafty's WASM module loads (and warms up) lazily inside the worker the
    // first time the modal is used; we just track "ready" optimistically so
    // the UI can show an engine-status hint. The worker itself posts a
    // 'ready' message once crafty.wasm has loaded — we don't strictly need
    // to listen for it here since the client module owns that, but we mark
    // ready after first mount so the badge isn't stuck on "loading" forever.
    useEffect(() => {
        const t = setTimeout(() => setEngineReady(true), 0);
        return () => clearTimeout(t);
    }, []);

    // Release the worker when the page unmounts so we don't leak a WASM
    ...
    // Crafty's worker already normalizes scores to White's perspective
    // (see crafty.worker.ts), so unlike a raw UCI score we don't need to
    // flip on side-to-move here — just pass it through.
    ...
    const handleExplainWithAI = async () => {
        ...
        const prompt = `You are a chess coach. The user has just analyzed a chess game using the Crafty engine. Below is the complete PGN followed by a move-by-move analysis summary.
...
Please explain the key moments of this game in a clear, educational way. Highlight critical mistakes, brilliant moves, tactical themes, and suggest improvements. Keep it engaging and suitable for a club-level player.`;
    ...
    return (
        <div className="bg-black text-white min-h-screen p-6 overflow-hidden relative">
            ...
            {/* Engine status badge */}
            <div className="fixed top-6 left-6 z-40 flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/50 border border-purple-500/30 backdrop-blur-sm">
                <FiZap className={`text-sm ${engineReady ? 'text-purple-400' : 'text-orange-400 animate-pulse'}`} />
                <span className="text-xs text-purple-100 font-medium">
                    Crafty {engineReady ? 'ready' : 'loading...'}
                </span>
            </div>

            {/* Floating upload button */}
            <motion.button
                className="fixed cursor-pointer top-6 right-6 z-40 p-3 bg-linear-to-r from-purple-600 to-indigo-600 rounded-full shadow-lg shadow-purple-500/30 hover:shadow-purple-500/50 transition-all"
            ...
            {analysisResults && !aiLoading && !aiExplanation && (
                <motion.button
                    className="fixed cursor-pointer top-24 right-6 z-40 p-3 bg-gradient-to-r from-[#FF4D00] to-[#FF0000] rounded-full shadow-lg shadow-[#FF4D00]/30 hover:shadow-[#FF4D00]/50 transition-all"
                    whileHover={{ scale: 1.1 }}
                    whileTap={{ scale: 0.9 }}
            ...
            {/* Upload PGN Modal (Crafty) */}
            <UploadPGNModalCrafty
            ...
            <div className="relative z-10 w-full max-w-7xl mx-auto space-y-6">
                ...
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
                    <h1 className="text-3xl md:text-4xl font-bold bg-linear-to-r from-purple-400 to-indigo-400 bg-clip-text text-transparent">
                        Game Analyzer — Crafty
                    </h1>
                    <p className="text-purple-200/70 text-sm mt-1">
                        Import or play moves to analyze positions with the Crafty WASM engine, running locally in your browser
                    </p>
                </motion.div>
                ...
                <div className="flex flex-col lg:flex-row gap-6 items-start justify-center">
                    ...
                    <div className="flex flex-col items-center gap-4 w-full lg:w-auto">
                        <div className="flex items-stretch gap-2 w-full justify-center">
                            <motion.div
                                ...
                                className="rounded-xl overflow-hidden shadow-2xl border border-purple-500/30 bg-black/30 aspect-square w-[90vw] md:w-125"
                            >
                                <Suspense
                                    fallback={
                                        <div className="aspect-square bg-black/30 flex items-center justify-center text-purple-200/70">
                                            Loading board...
                                        </div>
                                    }
                                >
                            ...
                            {/* Eval Bar */}
                            <div className="w-8 bg-black/30 rounded-r-xl border border-purple-500/30 border-l-0 overflow-hidden flex flex-col relative">
                        ...
                        {gameStatus && (
                            <motion.div
                                ...
                                className="p-4 rounded-xl bg-linear-to-br from-purple-600/20 to-indigo-600/20 border border-purple-500/30 text-center w-full max-w-[90vw] md:max-w-[500px]"
                            >
                                <div className="text-xl font-bold text-purple-100">{gameStatus}</div>
                            </motion.div>
                        )}
                    </div>
                    ...
                    <div className="flex-1 flex flex-col gap-4 w-full max-w-xl">
                        ...
                        <div className="flex justify-center gap-2">
                            <button onClick={goToStart} className="px-3 py-2 rounded-lg bg-black/40 border border-purple-500/30 hover:bg-purple-500/20 text-purple-100 font-bold transition" title="Go to start">
                                <FaStepBackward />
                            </button>
                            <button onClick={prevMove} className="px-3 py-2 rounded-lg bg-black/40 border border-purple-500/30 hover:bg-purple-500/20 text-purple-100 font-bold transition" title="Previous move">
                                <FaAngleLeft />
                            </button>
                            <button onClick={nextMove} className="px-3 py-2 rounded-lg bg-black/40 border border-purple-500/30 hover:bg-purple-500/20 text-purple-100 font-bold transition" title="Next move">
                                <FaAngleRight />
                            </button>
                            <button onClick={goToEnd} className="px-3 py-2 rounded-lg bg-black/40 border border-purple-500/30 hover:bg-purple-500/20 text-purple-100 font-bold transition" title="Go to end">
                                <FaStepForward />
                            </button>
                            <span className="self-center text-sm text-purple-300/70 ml-2">
                        ...
                        <motion.div
                            ...
                            className="p-4 rounded-2xl backdrop-blur-sm bg-linear-to-br from-purple-600/10 to-indigo-600/10 border border-purple-500/30"
                        >
                            <h2 className="text-lg font-bold text-purple-100 mb-3">Move History</h2>
                            <div className="max-h-64 overflow-y-auto space-y-1">
                                {moveHistory.length === 0 ? (
                                    <div className="text-center text-purple-200/60 py-8">No moves yet</div>
                                ) : (
                                    moveHistory.map((item, idx) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="w-8 text-purple-400 font-semibold">{item.moveNumber}.</div>
                                                <div className="flex-1 grid grid-cols-2 gap-2">
                                                    <div
                                                        className={`font-mono text-purple-100 px-2 py-1 rounded ${currentMoveIndex === whiteMoveIdx ? 'bg-purple-500/30' : ''}`}
                                                    >
                                                        {item.white || ''}
                                                    </div>
                                                    <div
                                                        className={`font-mono text-purple-100 px-2 py-1 rounded ${currentMoveIndex === blackMoveIdx ? 'bg-purple-500/30' : ''}`}
                                                    >
                                                        {item.black || ''}
                                                    </div>
                                                </div>
                        ...
                        <motion.div
                            ...
                            className="p-4 rounded-2xl backdrop-blur-sm bg-linear-to-br from-purple-600/10 to-indigo-600/10 border border-purple-500/30 flex-1"
                        >
                            <h2 className="text-lg font-bold text-purple-100 mb-3">Engine Analysis (Crafty)</h2>
                            ...
                            {analysisLoading && (
                                <div className="flex flex-col items-center justify-center py-8 space-y-3">
                                    ...
                                    />
                                    <p className="text-purple-200/70 text-sm">Running Crafty analysis in your browser...</p>
                                </div>
                            )}
                                ...
                                <div className="max-h-80 overflow-y-auto space-y-1">
                                    {analysisResults.map((line) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="w-12 text-purple-400 font-semibold">
                                                    {line.moveNumber === 0
                                                        ? 'Start'
                                                ...
                                                <div className="flex-1 flex items-center gap-2 min-w-0">
                                                    {line.san && (
                                                        <span className="font-mono text-purple-100 shrink-0">{line.san}</span>
                                                    )}
                                                ...
                                                <div className="hidden md:block text-xs text-purple-200/40 truncate max-w-37.5">
                                                    {line.pv.join(' ')}
                                                </div>
                                            </div>
                                        );
                        ...
                        {(aiLoading || aiExplanation || aiError) && (
                            <motion.div
                                initial={{ opacity: 0, y: 10 }}
                                animate={{ opacity: 1, y: 0 }}
                                className="p-4 rounded-2xl backdrop-blur-sm bg-gradient-to-br from-[#FF4D00]/10 to-[#FF0000]/10 border border-[#FF4D00]/30 flex-1"
                            >
                                <h2 className="text-lg font-bold text-orange-200 mb-3">AI Game Explanation</h2>
                                ...
                                {aiLoading && (
                                    <div className="flex flex-col items-center justify-center py-8 space-y-3">
                                        ...
                                        />
                                        <p className="text-orange-200/70 text-sm">AI is analyzing the game...</p>
                                    </div>
                                )}
                                ...
                                {aiExplanation && (
                                    <div className="max-h-96 overflow-y-auto">
                                        <div className="text-sm text-orange-100/90 whitespace-pre-wrap">{aiExplanation}</div>
                                    </div>
                                )}
                            </motion.div>
                        )}
                ...
                {showPromotionDialog && (
                    ...
                    >
                        <motion.div
                            ...
                            className="p-6 rounded-xl bg-linear-to-br from-purple-600/20 to-indigo-600/20 border border-purple-500/40 backdrop-blur-sm"
                        >
                            <h3 className="text-xl font-bold text-purple-100 mb-4 text-center">Choose Promotion</h3>
                            <div className="grid grid-cols-4 gap-4">
                                {[
                                    ...
                                    <motion.button
                                        ...
                                        className="p-6 rounded-xl bg-purple-600/20 hover:bg-purple-600/30 border border-purple-500/40 transition-colors"
                                    >
                                        <Icon className="text-4xl text-purple-100 mx-auto mb-2" />
                                        <div className="text-sm text-purple-200/70">{name}</div>
                                    </motion.button>
                                ))}
                            </div>
                        </motion.div>
...
export default GameAnalyzerCrafty;